...
local function finishLoading()
	...
	vape:Clean(playersService.LocalPlayer.OnTeleport:Connect(function()
		if (not teleportedServers) and (not shared.VapeIndependent) then
			...
			local teleportScript = [[
				loadstring(game:HttpGet('https://raw.githubusercontent.com/onnation/onnation/'..readfile('newvape/profiles/commit.txt')..'/loader.lua', true), 'loader')()
			]]
	...
	if not shared.vapereload then
		...
		if vape.Categories.Main.Options['GUI bind indicator'].Enabled then
			vape:CreateNotification('[AEROV4] Finished Loading', vape.VapeButton and 'Press the button in the top right to open GUI' or 'Press ' .. table.concat(vape.Keybind, ' + '):upper() .. ' to open GUI', 5)
		end
	end
end
